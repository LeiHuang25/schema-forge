import { type Context, createLens, setDefaultEngine } from "ldkit";
import { dbo, rdfs, xsd } from "ldkit/namespaces";
import {QueryEngine, SchemaPrototype} from "@comunica/query-sparql";
...
class Schema {
  ...
  private classMap: Map<string, SchemaPrototype> = new Map();
  ...
  public ClassSchema: SchemaPrototype
...
private async fetchSubclasses(classId: string) {
  ...
  private async fetchIncomingProperties(classId: string) {
    ...
    incomingPropsBindingsStream.on('data', (b) => {
    ...
    incomingPropsBindingsStream.on('error', (error) => {
      console.error(error);
    });
}
  private async fetchOutgoingProperties(classId: string) {
    ...
    outgoingPropsBindingsStream.on('data', (b) => {
      const property = b.get('property').value;
    ...
    outgoingPropsBindingsStream.on('error', (error) => {
      console.error(error);
    });
  }
  ...
  public getClassById(classId: string): SchemaPrototype {
    return this.classMap.get(classId);
  }
  public getClasses(): Array<SchemaPrototype> {
  ...
  public async getRelationDetails(relationIRI: string): Promise<{ sourceClass: SchemaPrototype, targetClass: SchemaPrototype, propertyLabel: string }> {
    try {
        const query = `
